...
2. 在GPT-SoVITS的主目录下执行脚本，导出所有需要的onnx模型，例如：`python GPT_SoVITS/export_onnx_v2.py --model_path ./models/v2 --export_name custom --version v2` 或者 `python GPT_SoVITS/export_onnx_v2.py --model_path ./models/v2pro --export_name custom --version v2Pro` 导出v2Pro模型。
